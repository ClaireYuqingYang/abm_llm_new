...
*Figure 2. Counterfactual response in fake-news reach using 60 real Twin-2K personas as the agent backbone. The hybrid ABM+LLM model moves when media literacy or story emotionality changes; the pure LLM-persona model is flat because its cached behavior profiles do not update when traits are perturbed.*
